...
mint <- "#B2EDC5"
meadow <- "#1EC296"
colors <- c("#418B82","#5F8DBF", "#255F85", "#B2EDC5")
ggthemr('fresh')
...
data <-
  map_dfr(c("PAYEMS", "UEMPLT5", "UEMP5TO14", "UEMP15T26", "UEMP27OV", "UNEMPLOY"), fredr) %>%
...
```{r dpi = 720, fig.align="center", echo = FALSE, warning = FALSE, message=FALSE}
  ...
  scale_color_manual("", labels = c("Unemployment rate", "Vacancy rate"), values = c(prussian, raspberry, babyblue)) +
  theme(text = element_text(color = "black", size = 10)) + 
  labs(y = "Percent \n",
       x = "", 
       fill = "", 
       title = "Figure 1, Unemployment, Vacany,and Job Finding Rate",
