...
data <-
  ...
  rename(short.term = UEMPLT5, 
         ...
         unemployed = UNEMPLOY) %>%
  mutate(date = ymd(date),
        f = 1 - ((unemployed - short.term)/lag(unemployed, n = 1))) %>%
...
Like the job finding rate, the vacany rate mirrors the unemployment rate.
...
which expresses the job finding probability as a function of the total unemployment level ("stock") and short term unemployment.
...
data %>%
  ...
  ggplot(aes(x = date, y = rollmean(f, k = 6, fill = NA))) +
  geom_line(color = sapphire) + 
  theme(panel.grid.major.x = element_blank(),
        panel.grid.major.y = element_blank(),
        legend.position = "bottom",
...
data %>%
  filter(date <= "2020-03-01") %>%
  ggplot(aes(x = date, y = rollmean(f - mean(f), k = 6, fill = NA))) +
  geom_line(color = sapphire) + 
  theme(panel.grid.major.x = element_blank(),
        panel.grid.major.y = element_blank(),
        legend.position = "bottom",
  ...
  scale_y_continuous(labels = percent_format(), position = "right") +
  theme(text = element_text(color = "black", size = 10)) + 
  labs(y = "Job finding rate (deviation from mean) \n", x = "", fill = "", title = "Figure 7",
       caption = "Source: Current Population Survey, Bureau of Labor Statistics")
```
...
data %>%
  ...
  theme(panel.grid.major.x = element_blank(),
        ...
        plot.title = element_text(face = "plain"),
        plot.caption = element_text(hjust = 1, size = 8)) +
  scale_x_date(breaks = as.Date(c("1990-01-01", "2000-01-01", "2010-01-01", "2020-01-01")),
               labels = date_format(format = "%Y")) +
  scale_y_continuous(labels = percent_format(), position = "right") +
  theme(text = element_text(color = "black", size = 10)) + 
  labs(y = "Job finding rate \n", x = "", fill = "", title = "Figure 9",
       caption = "Source: Current Population Survey, Bureau of Labor Statistics")
```
...
```{r dpi = 720, fig.align="center", echo = FALSE, warning = FALSE, message=FALSE}
 df %>%
  ...
  labs(y = "Percent \n", 
       x = "", 
       fill = "", 
       title = "Figure 8, Hires, Quits, & Layoffs (12-month rolling average)",
       caption = "Source: Bureau of Labor Statistics, Job Openings and Labor Turnover Survey")
```
...
df %>% 
  ...
  labs(y = "Percent \n", 
       x = "", 
       fill = "",
       title = "Figure 9, Hiring Rate (12-month rolling average)",
       caption = "Source: Bureau of Labor Statistics, Job Openings and Labor Turnover Survey")
```

Adjusting the window to the period after the Great Recession we can see the more recent trend. Smoothing the signal of the series further we can pick out the declining trend in the hiring rate. 
...
df %>% 
  ...
  labs(y = "Percent \n", 
       x = "", 
       fill = "",
       title = "Figure 10, Hiring Rate (24-month rolling average)",
       caption = "Source: Bureau of Labor Statistics, Job Openings and Labor Turnover Survey")
```
