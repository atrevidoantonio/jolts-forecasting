...
\Psi(L)y\_{t} = \delta + \Theta(L)\epsilon_{t} 
\end{gather*} 
$$
...
$$
\begin{gather*} 
y\_{t} = \delta + \sum\limits\_{i=0}^{p}\phi^{i}y_{t- i} + \epsilon_{t} - \sum\limits_{i=1}^{q}\theta_{i}\epsilon_{i-1} \\[8pt]
\left(1 - \sum\limits_{i=0}^{p}\phi^{i}L^{i}\right)y_{t} = \left(1 + \sum\limits_{i=1}^{q}\theta_{i}L^{i}\right)\epsilon\_{t} \\[8pt]
